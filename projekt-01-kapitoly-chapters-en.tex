...
% 
\begin{lstlisting}[xleftmargin=3.5ex,caption={Enabling accessibility via gsettings command},label={gsettings}]
...
A solution for those restriction is a custom implementation of tree. The implementations creates a tree from the original tree, with unusable nodes filtered out but preserving the parent-child relationship of the nodes. Furthermore, several important attributes are extracted in the process making them available to be used when the application instance has been terminated. This model will map possibilities of interactions available for user working with an application. It includes every node from the original tree that has an action available and is also executable by ATSPI. However the model does not allow execution of the actions directly. The instances of accessible objects are valid only for one application runtime,  Therefore, an execution of an action requires to use metadata stored in the model to find an instance of a node in the very last instance of the original application tree.
...
Implementation of the model consists of three classes and recreates the representation of the three created for requirements of this work. 

An instance of the class \texttt{GNode} represent a one node from the tree. Several attributes are copied from the original \texttt{dogtail.tree.Node} instance, including attributes storing informations about the parent node, data describing the state of the node, name of the action method, if available, and list of children. The list of children is composed also from instances of the \texttt{GNode} class, so the tree is recreated recursively. Therefore, the model is able to hold all information about tested applications, without relying on its state. Any \texttt{GNode} structure is not supposed to be used with a wrapper class \texttt{GTree}. An instance of the \texttt{GTree} can represent either a whole application or a smaller part of the application e.g.: a dialog or a menu. As discussed previously, this offline model of the application tree also contains a lot of nodes without ability of interaction, which need to be filtered out. List of \texttt{RoleName}s are gathered in the separate file \texttt{rolenames.py}. Then, the last class \texttt{TestTree} is filtering those nodes and preserving the parent-child relation ship. So, the result of this process is an instance of the \texttt{TestTree} object and it contains only nodes required to generate test cases.

\section{Application runtime}
Before generation process starts, it is necessary to have ability to monitor the state of tested applications, including start and stop operations. It is required, that a fresh instance of an application with a default configuration has to be run for every test case. Therefore, after each generated test case there must be a restart and a cleanup of the application. This can be summed up into three steps:
\begin{enumerate}
    \item perform a custom cleanup for an application
    \item make sure that the application is not running
    \item start the application
\end{enumerate}

\subsection{Configuration}
Assurance of compatibility with various applications across the GNOME ecosystem requires, that some metadata describing the tested application has to be provided before test generation process. The metadata are gathered in configuration file written in YAML\footnote{https://yaml.org/} language and only contains the most necessary information required to recognize tested applications. The only required field is \texttt{a11y\_app\_name}, which contains a string value describing a name of an application in the accessibility tree. Other values are not compulsory, but depending on the tested application, they might be required on achieve proper functionality. Others are not needed for the generation process but are required for the proper functionality of the generated tests in the CI environment.

\begin{lstlisting}[language=yaml,caption={apps.yaml},label={apps.yaml}]
...
describe yaml, compared which are required by the tool which by the ci

\subsection{Monitoring}

Monitoring the state of an application can be achieved in several ways and there are several factors to be monitored. Performing the check if an application is running can be done by checking UNIX pid of the application process, however it is better to rely on AT-SPI. If the application tree is not available, it can be certainly assumed that application instance is not running. This statement applies also vice versa, so an assertion that an application has started is achievable in a same way. The implementation takes advantage of dogtail's \texttt{tree.node.applications()} call, returning a list of applications currently exposed to the accessibility bus.

Furthermore, is it also necessary to perform certain checks during the time an application is being interacted with. Therefore, every tested application will be run as a subprocess, which allows to capture the output generated by tested applications to standard stream (\textit{stdout, stderrr}). Once the application has been terminated, it also allow us to check the return codes. The implementation relies on python's standard library \texttt{subprocess}. The output generated to standard stream is checked for errors defined in designated configuration file. In case that error occurs throughout the generation process, they are printed right away. The warning contains the number identifying the test in which the error occurred, a full error message and the return code, if available. All other captured messages e.g. warnings, deprecation messages from the GTK framework are saved to one log file, in a folder where the tests are generated. The messages are being appended, so the log file can be checked any time during the generation process. Every line contains the test number, so it can be easily determined when the message occurred.  

\section{OCR integration and optimization}
The main goal of the OCR integration in this work is to provide an additional level of verification of string values presented by applications and thus not rely purely on AT-SPI. However, the integration of the OCR into generated test cases has to be reliable to avoid false positive results from the tests. Therefore, the integration of Tesseract has to be properly test and the implementation has to contain image preprocessing optimizations and configuration to achieve specified goals.
Tesseract offers several options allowing to optimize test detection and analysis. It is assumed that most of the tested applications will use english language, therefore the dataset trained for the english language is used.

As discused in \ref{OCR_section}, Tesseract is less prone to errors when operating with images containing black text on white background. Therefore the safest option, is a conversion of images to grayscale. It also has to be considered that some applications are using darker themes or contain parts with different color schemes. To avoid adding additional requirement to the implementation, the string is always being searched in two images. The first one is a gray scale copy of the original image, the second one is inverted copy of the gray scale image. This ensures that the searched string is found on the screen, if present, regardless of theming of an applications and prevents false positive results during the test execution. An example is shown in Figure \ref{ocr_conversion}, containing 3 images, ordered from the top: an original image, a binarized image and an inverted binarized image. The Listings \ref{OCR_text2} is showing the result of the character recognition when image contains black text on white background. The result contains almost all strings shown in screen with some random characters created as an attempt to read icons located in the right top corner of the window. When compared to Listings \ref{OCR_text1}, where the source image contains white text on black background, it is proving an increase of the efficiency achieved after the optimization. The results obtained from the original image are quite similar to the results in Listing \ref{OCR_text1}. 

\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth,clip]{obrazky-figures/OCR_conversion.jpg}
	\caption{Steps of image preprocessing for Optical Character Recognition tool Tesseract, From the top: original image, binarized image, inverted binarized image}
	\label{ocr_conversion}
\end{figure}

\begin{lstlisting}[caption={Text generated from gray scale image in Figure \ref{ocr_conversion}},label={OCR_text1}]
...
[test@localhost ~]$ 
\end{lstlisting}
\begin{lstlisting}[caption={Text generated from inverted gray scale images in Figure \ref{ocr_conversion}},label={OCR_text2}]
...
\section{Testing}
test coverage and node covedage
