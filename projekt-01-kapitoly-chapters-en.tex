...
Initially, we tested a blank Evince instance without a test file. However, results have shown that a lot of widgets are not available, or disabled. Therefore, we decided to add a test file in a \texttt{.pdf} format as shown in Figure \ref{evince}. A name and a path of the test file is configurable via the \texttt{apps.yaml} file. The test file is used during the test case generation process as well as the test case execution process. An inclusion of the test file have unlocked the majority of disabled widgets and allowed us to use the full potential of our test generator. 
