...
\paragraph{Flatpak} is a technology\footnote{\url{https://flatpak.org/}} for building and distributing desktop applications on Linux. Flatpak aims to solve the problem with cross-platform distribution of packages on Linux, thus avoiding problems with different package managers used across Linux distributions. Applications, or so-called flatpaks, are delivered to users regardless of the lifecycle of the underlying Linux distribution. The system implements a set of sandboxing technologies, to isolate Flatpaks from each other and the system, thus providing security benefits to users~\cite{flatpak}.
...
In this section, we describe results achieved by our test generator when we tested LibreOffice applications. Moreover, we discuss the alteraations that were required to be implement within our test generator due to issues caused by the accessibility layer. 
...
The recommendations for testing application with are generator is to perform an initial shallow generator run with an application to quickly obtain executable test cases for the CI pipeline. Then, perform a non-shallow run to let our generator test as much as possible.

Tests for LibreOffice StartCenter were pushed to a CI environment in RedHat. A couple of test runs were performed to verify the sanity of the tests in tthe given environment.
