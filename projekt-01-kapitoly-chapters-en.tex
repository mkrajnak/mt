...
Flatpak is a technology for building and distributing desktop applications on Linux\footnote{\url{https://flatpak.org/}}. Flatpak aims to solve the problem with cross-platform distribution packaged on Linux, thus avoiding problems with different package managers used across Linux distributions. Applications, or so-called flatpaks, can be delivered to users regardless of the lifecycle of Linux distribution. The system implements a set of sandboxing technologies, to isolate flatpaks from each other and the system, thus providing security benefits to users.\cite{flatpak}
